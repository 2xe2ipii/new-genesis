...
export const DiscussionScreen: React.FC<DiscussionScreenProps> = ({ 
  ...
  // Resolve Target Name if card is used
  const targetName = (me.isCardUsed && me.cardTargetId) 
    ? room.players[me.cardTargetId]?.name 
    : null;
         ...
         <AnimatePresence mode="wait">
            {latestMessage ? (
              <motion.p 
                ...
                exit={{ opacity: 0, y: -10 }}
                // COLOR FIX: Check if message contains 'THREAT' or 'SAFE'
                className={`text-[10px] font-mono font-bold uppercase tracking-wider truncate ${
                    latestMessage.text.includes('THREAT') ? 'text-red-500 drop-shadow-[0_0_5px_rgba(239,68,68,0.5)]' : 
                    latestMessage.text.includes('SAFE') ? 'text-emerald-400' : 
                    'text-blue-300'
                }`}
              >
                {`>> ${latestMessage.text}`}
        ...
        <div className="grid grid-cols-3 gap-3">
          {playersList.map(p => {
            ...
            if (isTargeting) {
               ...
               // Not targeting? Only allow clicking self
               isInteractable = isMe;
            }
        ...
        {cardConfig && (
           ...
           >
             <div className="flex items-center gap-3 z-10">
               <div className={`p-2 rounded ${me.isCardUsed ? 'text-slate-600 bg-slate-900' : `${cardConfig.color} ${cardConfig.bg}/50 border border-white/10`}`}>
                 {cardConfig.icon}
               </div>
